# ENUNCIADO:
#
#    3) Desenvolva uma aplicação com OpenGL, com base no Ex1 - Renderização de um triângulo, de modo 
#       que no fragment shader a cor final do fragmento seja calculada com base na distância do 
#       fragmento em relação aos vértices do modelo do triângulo:
#       
#                = Se estiver mais próximo ao vértice de cima, o fragmento deve ser vermelho.
#                = Se estiver mais próximo ao vértice da direita, o fragmento deve ser verde.
#                = Se estiver mais próximo ao vértice da esquerda, o fragmento deve ser azul.
#
# =====================================================================================================
...
def inicializaShaders():
    ...
    fragment_shader = """
		...
		void main () {
            vec3 posicao_cima, posicao_direita, posicao_esquerda;            
            posicao_cima = vec3(0.0, 0.5, 0.0);
            posicao_direita = vec3(0.5, -0.5, 0.0);
            posicao_esquerda = vec3(-0.5, -0.5, 0.0);
            
            float distCima, distDireita, distEsquerda;
            distCima = distance(posicao_fragmento, posicao_cima);
            distDireita = distance(posicao_fragmento, posicao_direita);
            distEsquerda = distance(posicao_fragmento, posicao_esquerda);

		    if(distCima < distDireita && distCima < distEsquerda)
                frag_colour = vec4 (1.0, 0.0, 0.0, 1.0); // vermelho
            else if(distDireita < distCima && distDireita < distEsquerda)
                frag_colour = vec4 (0.0, 1.0, 0.0, 1.0); // verde
            else if(distEsquerda < distCima && distEsquerda < distDireita)
                frag_colour = vec4 (0.0, 0.0, 1.0, 1.0); // azul
            else
                frag_colour = vec4 (1.0, 0.0, 0.0, 1.0); // Retorna vermelho se o ponto for central
		}
    """
